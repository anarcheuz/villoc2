...
class Block(Printable):
    ...
    def __init__(self, obj, addr, size, error=False, tmp=False, **kwargs):
    ...
    def more_html(self):
        return "+ %#x (%#x)<br />[%s]" % (self.end() - self.start(), self.usize, self._obj)
...
class Marker(Block):
    ...
    def __init__(self, obj, addr, error=False, **kwargs):
        super().__init__(obj, addr, 0x0, tmp=True, error=error, *kwargs)

    def more_html(self):
        return "unknown"


def match_ptr(state, obj, ptr):
    ...
    if smallest_match is None:
        ...
        state.append(Marker(obj, ptr, error=True))

    return s, smallest_match


def malloc(state, obj, ret, size):

    if not ret:
        state.errors.append("Failed to allocate %#x bytes." % size)
    else:
        state.append(Block(obj, ret, size))


def calloc(state, obj, ret, nmemb, size):
    malloc(state, obj, ret, nmemb * size)


def free(state, obj, ret, ptr):

    if ptr is 0:
        return

    s, match = match_ptr(state, obj, ptr)

    if match is None:
        return
    elif ret is None:
        state[s] = Block(obj, match.uaddr, match.usize,
                         error=True, color=match.color)
    else:
        del state[s]


def realloc(state, obj, ret, ptr, size):

    if not ptr:
        return malloc(state, obj, ret, size)
    elif not size:
        return free(state, obj, ret, ptr)
    ...
    if match is None:
        ...
        state[s] = Block(obj, match.uaddr, match.usize, color=match.color)
        state[s].error = True
    else:
        state[s] = Block(obj, ret, size, color=match.color)
...
def parse_trace(trace):
    ...
    match_call = r"<([a-z_]+)> ([a-z_]+)\((.*)\) += (.*)"
    match_err = r"^([a-z_]+)\((.*) <no return \.\.\.>"

    for line in trace:

        try:
            obj, func, args, ret = re.findall(match_call, line)[0]
        except Exception:
...
def build_timeline(events):
    ...
    for obj, func, args, ret in events:
        try:
            op = operations[func]
        except KeyError:
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument("trace", type=argparse.FileType("rb"))
    ...
    noerrors = codecs.getreader('utf8')(args.trace.detach(), errors='ignore')
    timeline, boundaries = build_timeline(parse_trace(noerrors))
